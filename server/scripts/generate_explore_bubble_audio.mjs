// Generate the Explore mode-selector professor-dog speech-bubble clips in the
// SAME voice engine the app uses, and write them as MP3s straight into the iOS
// Resources folder (auto-bundled via the Xcode synchronized group).
//
...
//
// Output files (5 phrases × 2 languages):
//   explore_bubble_<i>_en.mp3   ES + "English meaning"
//   explore_bubble_<i>_uk.mp3   ES + "Ukrainian meaning"
//
...
//   railway run --service api node server/scripts/generate_explore_bubble_audio.mjs
//   railway run --service api node server/scripts/generate_explore_bubble_audio.mjs --lang=uk
//   railway run --service api node server/scripts/generate_explore_bubble_audio.mjs --only=2 --force
//
...
// Mirrors the `explorePhrases` table in Chat/Sessions/NewSessionView.swift (es +
// en) and the meaning values in Localization/Strings_uk_Home.swift (uk). The
// English meaning == the Swift `meaning` string, which is also the L() key.
const PHRASES = [
  { es: "¿Qué exploramos hoy?", en: "What are we exploring today?", uk: "Що досліджуємо сьогодні?" },
  { es: "¿Cómo quieres aprender?", en: "How do you want to learn?", uk: "Як хочеш учитися?" },
  { es: "¡Elige tu aventura!", en: "Pick your adventure!", uk: "Обери свою пригоду!" },
  { es: "¡Tú decides!", en: "You decide!", uk: "Тобі вирішувати!" },
  { es: "El mundo es tu aula.", en: "The world is your classroom.", uk: "Світ — твій клас." },
];
...
// us under the Gemini TTS free-tier rate/quota limit (bursting trips HTTP 429).
...
async function buildPhrase(index, phrase) {
  ...
  const targets = LANGS
    ...
    .map((lang) => ({ lang, outPath: join(OUT_DIR, `explore_bubble_${index}_${lang}.mp3`) }))
